    ...
    def run(self):
        ...
        for (id, url) in self.tasklist:
            ...
            try:
                ...
                result = response.text.encode('utf-8') #.replace('\n','')

                selector=etree.HTML(result, parser=None, base_url=None)

                #context=selector.xpath('//*[@id="useful"]/li/text()')
                context = selector.xpath('//div[@class="basic-info cmn-clearfix"]').extract()[0]
            ...
            try:
                sql_num = int(random.uniform(1, 8)) #随机一个限制数,200-300 到则进行插入
                if(i >= sql_num):
                    i = 0
